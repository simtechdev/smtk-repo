...
%if %{?scl:1}0
install -m 755 -d %{buildroot}%{_root_httpd_moddir}
ln -s %{_httpd_moddir}/libphp5.so      %{buildroot}%{_root_httpd_moddir}/lib%{name}5.so
%endif
...
install -m 644 %{SOURCE6} %{buildroot}%{_unitdir}/%{?scl_prefix}php-fpm.service
sed -e 's:/run:%{_localstatedir}/run:' \
    -e 's:/etc:%{_sysconfdir}:' \
    -e 's:/usr/sbin:%{_sbindir}:' \
    -i %{buildroot}%{_unitdir}/%{?scl_prefix}php-fpm.service
%else
# Service
install -m 755 -d %{buildroot}%{_root_initddir}
install -m 755 %{SOURCE11} %{buildroot}%{_root_initddir}/%{?scl_prefix}php-fpm
...
sed -e '/php-fpm.pid/s:/var:%{_localstatedir}:' \
    ...
    -e 's:/usr/sbin:%{_sbindir}:' \
    -i %{buildroot}%{_root_initddir}/%{?scl_prefix}php-fpm
%endif
# LogRotate
install -m 755 -d %{buildroot}%{_root_sysconfdir}/logrotate.d
install -m 644 %{SOURCE7} %{buildroot}%{_root_sysconfdir}/logrotate.d/%{?scl_prefix}php-fpm
sed -e 's:/run:%{_localstatedir}/run:' \
    -e 's:/var/log:%{_localstatedir}/log:' \
    -i %{buildroot}%{_root_sysconfdir}/logrotate.d/%{?scl_prefix}php-fpm
# Environment file
...
#ln -s %{_bindir}/php       %{buildroot}%{_root_bindir}/%{?scl_prefix}php
#ln -s %{_bindir}/phar.phar %{buildroot}%{_root_bindir}/%{?scl_prefix}phar
%endif
...
install -d %{buildroot}%{_macrosdir}
...
install -m 644 -c macros.php \
           %{buildroot}%{_macrosdir}/macros.%{_name}
...
%changelog
* Sat Jan 16 2016 Gleb Goncharov <yum@gongled.me> - 5.6.17-2
- Initial build.
...
- add php56-phpdbg package output
